...
async def list_portfolio(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def add_asset(
    ...
    service = PortfolioService(db)
    return service.create_asset(current_user.id, payload)


@router.patch("/{asset_id}", response_model=PortfolioAssetRead)
def update_asset(
    asset_id: uuid.UUID,
    ...
    service = PortfolioService(db)
    return service.update_asset(asset, payload)
